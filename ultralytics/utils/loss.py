...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max=16):
        ...
        # NWD
        self.nwd_loss = True
        self.iou_ratio = 0.3  # total_iou_loss = self.iou_ratio * iou_loss + (1 - self.iou_ratio) * nwd_loss
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask,
        ...
        # ========== 损失函数选择配置 ==========
        # 在这里修改 LOSS_TYPE 来切换不同的损失函数
        LOSS_TYPE = "Inner-MPDIoU"  # 可选: "CIoU", "Inner-IoU", "MPDIoU", "Inner-MPDIoU", "Focaler-IoU", "Focaler-MPDIoU"
        # ===================================
    ...
    def __call__(self, preds, batch):
        ...
        if isinstance(self.assigner, ATSSAssigner):
            anchors, _, n_anchors_list, _ = \
                generate_anchors(feats, self.fpn_strides, self.grid_cell_size, self.grid_cell_offset,
                                 device=feats[0].device)
            target_labels, target_bboxes, target_scores, fg_mask, target_gt_idx = self.assigner(anchors, n_anchors_list,
                                                                                                gt_labels, gt_bboxes,
                                                                                                mask_gt,
                                                                                                pred_bboxes.detach() * stride_tensor)
        else:
        ...
        if isinstance(self.bce, (nn.BCEWithLogitsLoss, FocalLoss_YOLO)):
            ...
            if fg_mask.sum():
                ...
                targets_onehot_pos = torch.zeros((target_labels.shape[0], target_labels.shape[1], self.nc),
                                                 dtype=torch.int64,
                                                 device=target_labels.device)  # (b, h*w, 80)
            loss[2] = self.bce(pred_scores, cls_iou_targets.to(dtype), targets_onehot_pos.to(torch.bool)).sum() / max(
                fg_mask.sum(), 1)
        ...
        if isinstance(self.bce, (nn.BCEWithLogitsLoss, FocalLoss_YOLO)):
            ...
            if fg_mask.sum():
                ...
                targets_onehot_pos = torch.zeros((target_labels.shape[0], target_labels.shape[1], self.nc),
                                                 dtype=torch.int64,
                                                 device=target_labels.device)  # (b, h*w, 80)
            loss[3] = self.bce(pred_scores, cls_iou_targets.to(dtype), targets_onehot_pos.to(torch.bool)).sum() / max(
                fg_mask.sum(), 1)
    ...
    def __call__(self, preds, batch):
        ...
        if isinstance(self.bce, (nn.BCEWithLogitsLoss, FocalLoss_YOLO)):
            ...
            if fg_mask.sum():
                ...
                pos_ious = probiou(pred_bboxes.reshape((b * n, c)), target_bboxes.reshape((b * n, c))).clamp(
                    min=1e-6).detach().reshape((b, n, 1))
                # 10.0x Faster than torch.one_hot
                cls_iou_targets = torch.zeros((target_labels.shape[0], target_labels.shape[1], self.nc),
                                              dtype=torch.int64,
                                              device=target_labels.device)  # (b, h*w, 80)
